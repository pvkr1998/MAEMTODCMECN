        ...
        for step in users['servers_set'][u]:
            ...
            if prev_node==-2:   #no previous => uplink + exec + downlink
                for s in step:
                    ...
                    if(min_exec > ener and time <= users['deadline'][u]):
                        min_exec = ener
                        arg_min = s
                if(min_exec != math.inf):
                    if(min_> min_exec):
                        min_ = min_exec
            else:
                for s in step:
            ...
            if(i<users['steps'][u]-1):
                ...
                for s in step:
                    ...
                    if datarate==0:
                        print("test\n",np.log2(1 + servers['transPow'][s]/(params.NOISE*pow(distance,params.ALPHA))),"\n",servers['transPow'][s],"\n",pow(distance,params.ALPHA),"\n")
